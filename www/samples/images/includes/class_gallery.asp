...
Class cls_gallery
    ...
    Public Function BuildThumbBytes(db, id)
        ...
        Set img = ASP4.image.Image.open(srcPath)
        w = ToInt(img.width, 0)
        h = ToInt(img.height, 0)
        maxDim = w
        If h > maxDim Then maxDim = h
    ...
    Public Function BuildFilterBytes(db, id, filterName)
        ...
        Select Case UCase(Trim("" & filterName))
            Case "BLUR": Set img = img.filter(ASP4.image.ImageFilter.BLUR)
            Case "CONTOUR": Set img = img.filter(ASP4.image.ImageFilter.CONTOUR)
            Case "DETAIL": Set img = img.filter(ASP4.image.ImageFilter.DETAIL)
            Case "EDGE_ENHANCE": Set img = img.filter(ASP4.image.ImageFilter.EDGE_ENHANCE)
            Case "EDGE_ENHANCE_MORE": Set img = img.filter(ASP4.image.ImageFilter.EDGE_ENHANCE_MORE)
            Case "EMBOSS": Set img = img.filter(ASP4.image.ImageFilter.EMBOSS)
            Case "FIND_EDGES": Set img = img.filter(ASP4.image.ImageFilter.FIND_EDGES)
            Case "SHARPEN": Set img = img.filter(ASP4.image.ImageFilter.SHARPEN)
            Case "SMOOTH": Set img = img.filter(ASP4.image.ImageFilter.SMOOTH)
            Case "SMOOTH_MORE": Set img = img.filter(ASP4.image.ImageFilter.SMOOTH_MORE)
            Case Else
