...
If errMsg = "" And UCase("" & Request.ServerVariables("REQUEST_METHOD")) = "POST" Then
    ...
    Else
        ...
        If uid <= 0 Then
            Call PortalExec(db, "INSERT INTO users(tenant_id,name,email,password_hash,role,status,timezone) VALUES(" & tenantId & "," & SqlQ(nameVal) & "," & SqlQ(email) & "," & SqlQ(ASPpy.Crypto.Hash(pass1,10)) & "," & SqlQ(roleName) & ",'active','UTC')")
        ...
        Else
            Call PortalExec(db, "UPDATE users SET name=" & SqlQ(nameVal) & ",password_hash=" & SqlQ(ASPpy.Crypto.Hash(pass1,10)) & ",status='active',role=" & SqlQ(roleName) & " WHERE id=" & uid)
        End If
