...
## First: which setup are you in?

ASPPY runs two ways, and the commands differ. Work out which one applies before
following anything below.

**Installed with pip** (`pip install asppy`) - there is no `www`, no
`www_starter` and no `ASPPY\` folder in your project. The runtime lives in
Python's `site-packages`, and you get five commands on your PATH:

| Command | Purpose |
|---|---|
| `asppy-new myapp` | create a new app from the starter template - **always start here** |
| `asppy 127.0.0.1 8080 myapp` | run the dev server on your app folder |
| `asppy-render myapp/default.asp` | render one page to stdout, no server needed |
| `asppy-check myapp` | render every page and report failures |
| `asppy-guide` | print this document; `asppy-guide --list` shows all bundled docs |

**Working in a git checkout** of the ASPPY repo - you have `www`, `www_starter`,
`www_test` and `ASPPY\*.py` on disk. Use the repo-relative forms:

| Checkout form | pip equivalent |
|---|---|
| copy `www_starter` to `www` | `asppy-new myapp` |
| `python -m ASPPY.server 0.0.0.0 8080 www` | `asppy 0.0.0.0 8080 myapp` |
| `python asppycli.py www/default.asp` | `asppy-render myapp/default.asp` |
| `python asppycheck.py www` | `asppy-check myapp` |

Everything below is written in the checkout style, using `www` as the app folder.
**If you installed with pip, read `www` as "your app folder"** and use the
right-hand column above. The rules, conventions and idioms are identical either
way - only the paths and command names change.

Two of the hard rules below name repo paths. With a pip install, "do not modify
`ASPPY\*.py`" means never edit the runtime inside `site-packages` - that would
break every ASPPY app on the machine - and "do not modify `www_starter`" means
never edit the template inside the installed package. Work only in the folder
`asppy-new` created for you.

## Start from the starter

Start every new app from the starter template - `www_starter` in a checkout, or
`asppy-new myapp` with a pip install.

- Do not begin from a blank folder when creating a new app
- Copy the starter first, then build from that structure
- The rules below still apply to the finished app
