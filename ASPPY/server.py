...
from ASPPY.global_asa import compile_global_asa, GlobalAsaCompiled
from ASPPY.http_response import RenderResult, Response, ResponseEndException
from ASPPY.server_object import Server, ServerTransferException
from ASPPY.asp_include import IncludeError
...
def _try_front_controller(docroot: str, url_path: str):
    normalized = url_path or '/'
    if not normalized.startswith('/'):
        normalized = '/' + normalized

    ext = os.path.splitext(normalized.rstrip('/'))[1].lower()
    if ext:
        return None
    if normalized.lower() == '/default.asp':
        return None

    try:
        front_phys = _safe_join(docroot, '/default.asp')
    except PermissionError:
        return None

    if os.path.isfile(front_phys):
        return '/default.asp', front_phys

    alt_front = _resolve_case_insensitive_path(docroot, '/default.asp')
    if alt_front and os.path.isfile(alt_front):
        return _phys_to_url(docroot, alt_front), alt_front
...
def _run_global_asa_body(app_root: str, app_store: ApplicationStore, body: str, sess=None):
    ...
    from ASPPY.vm.interpreter import VBInterpreter
    from ASPPY.runner_vm import _build_globals_env
    from ASPPY.http_request import Request as Req
    from ASPPY.server_object import Server as Srv
...
class ASPRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle(self):
        ...
        # Redirect /foo to /foo/ only if it maps to a directory that has a
        # servable default document.  Empty directories (no default document)
        # fall through so the front-controller can handle extensionless URLs
        # like /admin even when an empty /admin/ folder exists on disk.
        if not path.endswith('/'):
            _dir_redirect_phys = None
            try:
                _dir_redirect_phys = _safe_join(docroot, path)
            except PermissionError:
                pass
            if _dir_redirect_phys and os.path.isdir(_dir_redirect_phys):
                _dir_dd = _try_default_document(docroot, path + '/')
                if _dir_dd is not None:
                    _redir_loc = path + '/'
                    if request_query:
                        _redir_loc += '?' + request_query
                    self.send_response(301)
                    self.send_header('Location', _redir_loc)
                    self.send_header('Content-Length', '0')
                    self.send_header('Connection', 'keep-alive')
                    self.end_headers()
                    return
                # else: directory exists but has no default doc, skip redirect
        ...
        if not os.path.isfile(full_path):
            ...
            if alt_full and os.path.isfile(alt_full):
                ...
                front = _try_front_controller(docroot, request_path)
                if front is None:
                    self.send_error(404, "ASP page not found")
                    return
                exec_path, full_path = front
        ...
        if not is_head:
            ...
            def _consume(part: bytes):
                ...
                if (mem_total + len(part)) <= mem_limit:
                    ...
                    mem_total += len(part)
                    return
                fd, body_file_path = tempfile.mkstemp(prefix="ASPPY_req_", suffix=".bin")
                os.close(fd)
        ...
        if _env_bool("ASP_PY_TRACE_REQUEST", False):
            try:
                ...
                print(
                    f"[ASPPY trace] {self.command} {path} ctype={ctype!r} content_length={headers.get('Content-Length') or headers.get('content-length')!r} body_len={body_len} form_keys={list(form_map.keys())}",
                    file=sys.stderr,
                )
                if method not in ("GET", "HEAD"):
                    print(f"[ASPPY trace] body_preview={body_preview_txt!r}", file=sys.stderr)
                    ...
                    for k in ("aspFormAction", "yesno", "checkbox", "radio"):
                        try:
                            ...
                            v = f"<error {e}>"
                        print(f"[ASPPY trace] form[{k}]={v!r}", file=sys.stderr)
            except Exception:
                pass
